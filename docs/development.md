...
> 每個 preview/deploy 環境是獨立的 compose project（`fullstack-dev` / `fullstack-qas` / `fullstack-prod`）；`make dev` 則以資料夾名當 project 名。用 `docker compose ls` 對照 project 名，就知道哪個 topology/環境正開著。
